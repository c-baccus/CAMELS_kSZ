...
# ---- Configuration ----
CAMELS_BASE = "/scratch/cjb9346/camels"
OUTPUT_BASE = "outputs"
SNAPSHOT_NUM = 74
N_GAL = 100
HUBBLE_H = 0.6711
...
def process_one_sim(cv_index):
    """Run the full pipeline (select + make maps) on one CV sim."""
    snap_dir = os.path.join(CAMELS_BASE, f"CV_{cv_index}/snapdir_074")
    group_dir = os.path.join(CAMELS_BASE, f"CV_{cv_index}/groups_074")
    output_dir = os.path.join(OUTPUT_BASE, f"CV_{cv_index}")
...
def main():
    ...
    parser.add_argument('--sims', type=str, default=None,
                        help='Comma-separated list of CV indices (e.g., "1,2,5,10"). Default: all 0-26.')
    args = parser.parse_args()

    if args.sims:
        sim_indices = [int(s) for s in args.sims.split(',')]
    else:
        sim_indices = list(range(27))  # CV_0 through CV_26

    print(f"Will process sims: {sim_indices}")
    overall_t0 = time.time()

    for cv_idx in sim_indices:
        # Skip if outputs already exist (unless --force)
        maps_path = os.path.join(OUTPUT_BASE, f"CV_{cv_idx}", "all_maps.npz")
        if os.path.exists(maps_path) and not args.force:
            print(f"\nCV_{cv_idx}: outputs exist, skipping (use --force to redo)")
            continue

        # Verify the input data exists
        snap_dir = os.path.join(CAMELS_BASE, f"CV_{cv_idx}/snapdir_074")
        group_dir = os.path.join(CAMELS_BASE, f"CV_{cv_idx}/groups_074")
        if not os.path.isdir(snap_dir) or not os.path.isdir(group_dir):
            print(f"\nCV_{cv_idx}: data not found, skipping")
            continue

        try:
            process_one_sim(cv_idx)
        except Exception as e:
            print(f"\n!!! Error processing CV_{cv_idx}: {e}")
            import traceback
            traceback.print_exc()
            continue
